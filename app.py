...
def insert():
    ...
    if request.method == "POST":
        ...
        cur.execute("""
               UPDATE eventss
               SET event_name=%s, event_type=%s, start_date=%s, end_date=%s, fee=%s
               WHERE id=%s
            """, (event_name, event_type,start_date,end_date,fee,id))
        mysql.connection.commit()
...
def view():
	slno=request.form['slno']
	cur = mysql.connection.cursor()
	cur.execute("select desc FROM eventss where slno=%s",(slno))
	mysql.connection.commit()
	return redirect(url_for('index2'))
...
def update():
    ...
    if request.method == 'POST':
        ...
        cur.execute("""
               UPDATE eventss
               SET event_name=%s, event_type=%s, start_date=%s, end_date=%s, fee=%s, desc=%s
               WHERE slno=%s
            """, (event_name, event_type,start_date,end_date,fee,desc,slno))
